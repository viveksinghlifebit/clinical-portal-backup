...
export const loadEndpoints = async (
  ...
  for (const endpoint of Object.values(endpointsInfo)) {
    router[endpoint.method](
      ...
      (ctx: Koa.ParameterizedContext<App.State, App.Context>) => openApi.handleRequest(ctx.request, ctx)
    )
  }
...
const _getOperationMapFromEndpointsInfoMap = (EndpointsInfo: App.EndpointsInfo): { [id: string]: OpenApiBackend.Handler } =>
  Object
    .keys(EndpointsInfo)
    .reduce(
      (accm: { [id: string]: OpenApiBackend.Handler }, key: keyof App.EndpointsInfo) =>
        ({ ...accm, [key]: _wrapOpenApiContext(EndpointsInfo[key]!.operation) }),
      {}
    )

const _wrapOpenApiContext = (operation: App.EndpointOperation): OpenApiBackend.Handler =>
  (_: OpenApiBackend.Context, ctx: Koa.ParameterizedContext<App.State, App.Context>) => operation(ctx)
